...
export default async function RequestDetailPage({
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-3">
          {(request.service_request_answers ?? []).map((answer: any) => (
            <div key={answer.id} className="rounded-xl border border-slate-200 p-3">
              ...
              <p className="font-medium text-slate-900">{answer.field_value || '-'}</p>
            </div>
          ))}
        </div>
      </Card>
      ...
      <Card title="Dokumen Persyaratan">
        <div className="space-y-4">
          ...
          {!request.service_request_documents?.length ? (
            <p className="text-sm text-slate-500">Belum ada dokumen terupload.</p>
          ) : null}
        </div>
      </Card>
        ...
        <Card title="Upload Revisi">
          <div className="grid gap-4 md:grid-cols-2">
            {(requirements ?? []).map((requirement: any) => (
              <UploadRevisionForm key={requirement.id} requestId={request.id} requirement={requirement} />
            ))}
          </div>
        </Card>
      ) : null}
      ...
      <Card title="Dokumen Hasil">
        {generatedUrl ? (
          ...
          <p className="text-sm text-slate-500">
            Dokumen hasil belum tersedia. Dokumen akan muncul setelah admin menyetujui dan generate PDF.
          </p>
        )}
      </Card>
      ...
      <Link href="/dashboard/pengajuan" className="text-sm font-medium text-green-700">
